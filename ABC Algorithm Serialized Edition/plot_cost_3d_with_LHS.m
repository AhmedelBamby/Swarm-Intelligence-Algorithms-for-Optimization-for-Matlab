...
%{
numSamples = 1000;   % Number of LHS samples

% Define sampling ranges for k1, k2, k3, k4
ranges = [0, 6000]; % Same range for all parameters

% Generate LHS samples (more uniform coverage than random)
lhs_samples = lhsdesign(numSamples, 4, 'iterations', 50, 'criterion', 'maximin'); % 4 parameters
...
%}
%% ============================== Sampling Controller Gain using LHS with Manual Ranges ==================================
numSamples = 400;   % Number of LHS samples

% Define individual sampling ranges for each parameter
% Format: [min, max] for each k
k1_range = [0.055, 1.2575];   % Example range for k1
k2_range = [2, 4];      % Example range for k2
k3_range = [110, 110.30];    % Example range for k3
k4_range = [220, 600];     % Example range for k4

% Generate LHS samples in unit hypercube
lhs_samples = lhsdesign(numSamples, 4, 'iterations', 50, 'criterion', 'maximin');

% Scale each parameter to its specific range
k1_samples = k1_range(1) + (k1_range(2)-k1_range(1)) * lhs_samples(:,1);
k2_samples = k2_range(1) + (k2_range(2)-k2_range(1)) * lhs_samples(:,2);
k3_samples = k3_range(1) + (k3_range(2)-k3_range(1)) * lhs_samples(:,3);
k4_samples = k4_range(1) + (k4_range(2)-k4_range(1)) * lhs_samples(:,4);

% Combine all samples into a single matrix
data = [k1_samples, k2_samples, k3_samples, k4_samples];

% Optional: Visualize parameter distributions
figure;
subplot(2,2,1); histogram(k1_samples); title('k1 distribution'); xlabel('k1 range');
subplot(2,2,2); histogram(k2_samples); title('k2 distribution'); xlabel('k2 range');
subplot(2,2,3); histogram(k3_samples); title('k3 distribution'); xlabel('k3 range');
subplot(2,2,4); histogram(k4_samples); title('k4 distribution'); xlabel('k4 range');
...
parfor i = 1:numSamples
    ...
    fprintf("Done: %i out of %i",i,numSamples);
end
